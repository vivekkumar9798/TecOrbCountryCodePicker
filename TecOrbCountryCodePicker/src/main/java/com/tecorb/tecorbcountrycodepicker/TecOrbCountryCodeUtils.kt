...
object TecOrbCountryCodeUtils {
    private var countries: MutableList<TecOrbCountry>? = null
    private var timeZoneAndCountryISOs: MutableMap<String, List<String>>? = null

    fun getFlagDrawableResId(country: TecOrbCountry): Int {
     ...
     */
    fun getAllCountries(context: Context): List<TecOrbCountry> {
        if (countries != null) return countries!!

        countries = ArrayList<TecOrbCountry>()
        countries!!.add(
            TecOrbCountry(
        ...
        countries!!.add(TecOrbCountry(context.getString(R.string.country_ethiopia_code),
        ...
        countries!!.sortWith(compareBy { it.name.lowercase() })
        return countries!!
    }

    fun getByNumber(
        context: Context,
        preferredCountries: List<TecOrbCountry>?,
        fullNumber: String,
    ): TecOrbCountry? {
        if (fullNumber.length == 0) return null

        val firstDigit = if (fullNumber[0] == '+') {
            1
        } else {
            0
        }
        var country: TecOrbCountry?
    ...
    fun getByCode(context: Context, preferredCountries: List<TecOrbCountry>, code: Int): TecOrbCountry? {
        return getByCode(context, preferredCountries, code.toString())
    }

    private fun getByCode(context: Context, preferredCountries: List<TecOrbCountry>?, code: String): TecOrbCountry? {
        preferredCountries?.let {
            for (country in it) {
        ...
        // Check in all countries
        for (country in TecOrbCountryCodeUtils.getAllCountries(context)!!) {
            if (country.phoneCode == code) {
                return country
            }
        }
    ...
    fun getByNameCodeFromCustomCountries(
        context: Context,
        customCountries: List<TecOrbCountry?>?,
        nameCode: String?,
    ): TecOrbCountry? {
    ...
    fun getByNameCodeFromAllCountries(context: Context, nameCode: String?): TecOrbCountry? {
        val countries: List<TecOrbCountry>? = getAllCountries(context)
    ...
    private fun getTimeZoneAndCountryISOs(context: Context): Map<String, List<String>>? {
        ...
        timeZoneAndCountryISOs = HashMap()
        ...
        try {
            bufferedReader.useLines { lines ->
                lines.forEach { line ->
                    ...
                    if (!line.startsWith("#")) {
                        val wordsArray = line.split("\t").filter { it.isNotEmpty() }
                        if (wordsArray.size >= 3) {
                            val countryCodes = wordsArray[0].split(",").filter { it.isNotEmpty() }
                            val timeZone = wordsArray[2]

                            (timeZoneAndCountryISOs as HashMap<String, List<String>>)[timeZone] = countryCodes
                        }
                    }
                }
            }
